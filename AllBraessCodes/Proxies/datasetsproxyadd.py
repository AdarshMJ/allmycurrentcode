...
from listedproxyaddK import*
#from listedproxydelete import*

import logging
logger = logging.getLogger()
fhandler = logging.FileHandler(filename='experiments/ProxyAdditions/romanempireproxyadditions.log', mode='a')
...
class Dataset:
    def __init__(self, name, add_self_loops=False, device='cpu', use_sgc_features=False, use_identity_features=False,
        ...
        logging.info(f"Adding {max_iters} edges using Proxy Spectral Gap")
        start_algo = time.time()
        newgraph = add_and_update_edges(nxgraph,max_iterations)
        end_algo = time.time()
        logging.info(f"Time Taken for adding edges -- {end_algo - start_algo}")
        logging.info("")
        print("Done!")
        graph = dgl.from_networkx(nxgraph)
        #graph = dgl.graph((newadj.nonzero()[0], newadj.nonzero()[1]), num_nodes=len(node_features),idtype=torch.int)
       # graph = dgl.from_networkx(nx.from_scipy_sparse_array(newadj))
        print("After adding edges",graph)
