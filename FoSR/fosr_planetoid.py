import argparse
parser = argparse.ArgumentParser(description='Run FoSRGCNWebKB script')
parser.add_argument('--filename', type=str, help='Dataset to download')
parser.add_argument('--out', type=str, help='name of log file')
parser.add_argument('--max_iters', type=int, default=10, help='maximum number of FoSR iterations')
parser.add_argument('--dropout', type=float, default=0.5130296, help='Dropout = [0.21-0.51]')
parser.add_argument('--hidden_dimension', type=int, default=128, help='hidden_dimension = [32,64,128]')
...
after_score = homophily(data.edge_index, data.y, method='edge_insensitive')
logging.info(f"Homophilly score after = {after_score}")
logging.info("")
