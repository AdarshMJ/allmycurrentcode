...
fhandler = logging.FileHandler(filename='experiments/minesweeperfosr.log', mode='a')
...
class Dataset:
    def __init__(self, name, add_self_loops=False, device='cpu', use_sgc_features=False, use_identity_features=False,
        ...
        pygraph = from_networkx(nxgraph)
        print(pygraph)
        print("Done!")
        print("Performing FoSR Minimization...")
        logging.info(f"FoSR Iterations = {max_iters}")
        start_algo = time.time()
        logging.info(f"=========="*10)
        for j in tqdm(range((max_iters))):
              edge_index,edge_type,_,prod = fosr.edge_rewire(pygraph.edge_index.numpy(), num_iterations=1)      
              pygraph.edge_index = torch.tensor(edge_index)
        pygraph.edge_index = torch.cat([pygraph.edge_index])
        end_algo = time.time()
        logging.info(f"Time Taken for Pruning -- {end_algo - start_algo}")
        logging.info("")
        backnxgraph = nx.to_networkx(pygraph)
        graph = dgl.from_networkx(backnxgraph)
